...
def read_stage(cur, stage_table, recon_maps, stage_filter=None):
    ...
    (This is the lightweight alternative to stage_post_sql: an expression evaluated
    at read time rather than a statement run against the staging table.)
...
def reconcile_eligibility(cur, cfg, run_id, rows, target_cols):
    ...
    # member appears several times. Sorting by MEMBER_FROM_DATE first makes
    # "last wins" mean "latest span" rather than "whatever order the file was in".
    # Rows with an unparseable start date sort earliest so a real date beats them.
...
def run_config(cn, cfg):
    ...
    try:
        ...
        if cfg["feed_type"] == "Eligibility":
            ...
            # roster: everyone who termed between the two still appears, so nobody
            # is inactivated and members dropped by the newer file are resurrected
            # as active. Only the newest file is current -- older ones are recorded
            # as superseded so they are not picked up again.
            if len(todo) > 1:
                ordered = sorted(todo, key=lambda n: listed.get(n) or datetime.min)
                superseded, todo = ordered[:-1], [ordered[-1]]
                for name in superseded:
                    cur.execute("DELETE FROM dbo.Import_Processed_Files WHERE config_id=? AND file_name=?",
                                cfg["id"], name)
                    cur.execute(
                        "INSERT INTO dbo.Import_Processed_Files (config_id, file_name, rows_imported, file_modified) "
                        "VALUES (?,?,0,?)", cfg["id"], name, listed.get(name))
